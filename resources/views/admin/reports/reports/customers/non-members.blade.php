...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item text-muted">Non-Members Report</li>
@endsection
@section('actions')
...
@section('scripts')
    <script>
        $(document).ready(function(){
                ...
                columnDefs: [
                    {
                        ...
                        render: function(data, type, full){
                            return `<span class="fw-bolder fs-7">${full.son} </span>`;
                        }
                    },
                    {
                        targets: 1,
                        render: function(data, type, full){
                            return `<span class="fw-semibold">${moment(full.created_at).format('MMMM DD YY, h:mm:ss a')} </span>`;
                        }
                    },
                    {
                        targets: -1,
                        render: function(data, type, full){
                            return `<span class="fw-bold fs-7">${numberWithCommas(full.total)} </span>`;t
                        }
                    }
                ],
                ajax: {
                    ...
                    url: '{{ route('customers.non-members.report.table') }}',
            ...
            let documentTitle = `Non-Members Report `;
